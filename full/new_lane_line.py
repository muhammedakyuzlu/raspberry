...
def sliding_window(img, nwindows=30, margin=50, minpix = 1, draw_windows=True):
...
def get_curve(img, leftx, rightx):
    ploty = np.linspace(0, img.shape[0]-1, img.shape[0])
    y_eval = np.max(ploty)
    ym_per_pix = 1/480 #30.5/720 # c meters per pixel in y dimension
    xm_per_pix = 0.5/480 #3.7/720 # c  meters per pixel in x dimension
...
def draw_lanes(img, left_fit, right_fit):
    ...
    sss = cv2.addWeighted(img, 1, color_img, 0.5, 0)
    return sss#inv_perspective
...
def vid_pipeline(img):
    ...
    cv2.putText(img, 'Vehicle offset: {:.4f} cm'.format(curverad[2]), (570, 650), font, fontSize, fontColor, 2)
    return img


#https://drive.google.com/file/d/1qGTMOxgmRq1tX7Ag_zOn8gmGA_GrlLSG/view?usp=sharing
